...
$id = $_POST['idkejadian'];
$sql1 = "SELECT * FROM kejadian WHERE idkejadian='$id'";
$result = $c->query($sql1);
...
if($result->num_rows >0){
	...
	while($obj = $result->fetch_assoc()){
		...
		$sqlCK = 
		"SELECT * FROM komen_kejadian WHERE idkejadian = '$idkej'";
		$resultKomen = $c->query($sqlCK);
		if($objKomen = $resultKomen->fetch_assoc()){
			$kejadian['kejadians'][$i]['komenuser'] = addslashes(htmlentities($objKomen['username']));
			$kejadian['kejadians'][$i]['komentar'] = addslashes(htmlentities($objKomen['komentar']));
		}
